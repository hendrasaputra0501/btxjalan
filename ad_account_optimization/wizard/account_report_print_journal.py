...
class account_print_journal(osv.osv_memory):
    ...
    def _print_report(self, cr, uid, ids, data, context=None):
        ...
        data['form'].update(self.read(cr, uid, ids, ['sort_selection','type'], context=context)[0])
        if context.get('sale_purchase_only'):
            if data['form']['type'] == 'pdf':
                report_name = 'account.journal.period.print.sale.purchase.optim'
            else:
                report_name = 'account.journal.period.print.sale.purchase.optim.xls'
        else:
            if data['form']['type'] == 'pdf':
                report_name = 'account.journal.period.print.optim'
            else:
                report_name = 'account.journal.period.print.optim.xls'
