...
class journal_print(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        ...
        self.localcontext.update({
            ...
            'get_journal_name': self._get_journal_name,
    })
    ...
    def lines(self, period_id, journal_id=False):
        ...
        self.cr.execute('update account_journal_period set state=%s where journal_id IN (1) and period_id=%s and state=%s', ('printed', period_id, 'draft'))
    ...
    def _get_period2(self, data):
        obj_mline = self.pool.get('account.period')
        return obj_mline.browse(self.cr, self.uid, data)
    
    def _get_period(self, data):
        obj_mline = self.pool.get('account.move.line')
        self.cr.execute('SELECT distinct period_id FROM account_move_line where period_id IN %s ',(tuple(data),))
        ids = map(lambda x: x[0], self.cr.fetchall())
        return ids
    
    def _get_journal_name(self, data):
        return self.pool.get('account.journal').browse(self.cr, self.uid, data).name
    
    def _get_journal(self, data):
        obj_mline = self.pool.get('account.move.line')
        self.cr.execute('SELECT distinct journal_id FROM account_move_line where journal_id IN %s ',(tuple(data),))
        ids = map(lambda x: x[0], self.cr.fetchall())
        return ids
...
report_sxw.report_sxw('report.account.journal.period.print.mirror', 'account.journal.period', 'addons/ad_account_optimization/report/account_journal.rml', parser=journal_print, header='internal')
report_sxw.report_sxw('report.account.journal.period.print.sale.purchase.mirror', 'account.journal.period', 'addons/ad_account_optimization/report/account_journal_sale_purchase.rml', parser=journal_print, header='internal')
