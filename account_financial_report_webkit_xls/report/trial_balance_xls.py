...
from account_financial_report_webkit.report.trial_balance \
...
class trial_balance_xls(report_xls):
    column_sizes = [12, 12, 60, 17, 17, 17, 17, 17, 17]
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for current_account in  objects:
            ...
            c_specs = [
                ...
                ('code2', 1, 0, 'text', current_account.code2),
                ('account', account_span, 0, 'text', current_account.name),
            ]
            if _p.comparison_mode == 'no_comparison':

                debit_cell = rowcol_to_cell(row_pos, 5)
                credit_cell = rowcol_to_cell(row_pos, 6)
                bal_formula = debit_cell + '-' + credit_cell

                if _p.initial_balance_mode:
                    init_cell = rowcol_to_cell(row_pos, 4)
                    debit_cell = rowcol_to_cell(row_pos, 5)
                    credit_cell = rowcol_to_cell(row_pos, 6)
