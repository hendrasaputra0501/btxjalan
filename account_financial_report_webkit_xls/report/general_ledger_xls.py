...
from account_financial_report_webkit.report.general_ledger \
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for account in objects:
            ...
            if _p.display_account_raw(data) == 'all' or \
                ...
                # sum debit,credit, and balance on selected period
                debit_start = rowcol_to_cell((display_initial_balance and row_start+1 or row_start), 9)
                debit_end = rowcol_to_cell(row_pos - 1, 9)
                debit_formula = 'SUM(' + debit_start + ':' + debit_end + ')'
                credit_start = rowcol_to_cell((display_initial_balance and row_start+1 or row_start), 10)
                ...
                # sum cumulative debit.credit, and balance as on selected last period
                debit_start = rowcol_to_cell(row_start, 9)
                debit_end = rowcol_to_cell(row_pos - 2, 9)
                debit_formula = 'SUM(' + debit_start + ':' + debit_end + ')'
                credit_start = rowcol_to_cell(row_start, 10)
                credit_end = rowcol_to_cell(row_pos - 2, 10)
                credit_formula = 'SUM(' + credit_start + ':' + credit_end + ')'
                balance_debit = rowcol_to_cell(row_pos, 9)
                balance_credit = rowcol_to_cell(row_pos, 10)
                balance_formula = balance_debit + '-' + balance_credit
