...
class purchase_order_line_knock_off(osv.osv_memory):
	"""
	This wizard will confirm the all the selected draft invoices
	"""

	_name = "purchase.order.line.knock.off"
	...
	def order_knock_off(self, cr, uid, ids, context=None):
		...
		pool_obj = self.pool.get('purchase.order.line')
		...
		for line in pool_obj.browse(cr, uid, context['active_ids'], context=context):
			ctx={'date_knock_off': date}
			pool_obj.button_knock_off(cr, uid, [line.id], context=ctx)
			
		return {'type': 'ir.actions.act_window_close'}

purchase_order_line_knock_off()
