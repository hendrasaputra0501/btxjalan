...
class stock_ageing_parser(report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context=None):
		super(stock_ageing_parser, self).__init__(cr, uid, name, context=context)
		...
		if internal_type.code=='Finish':
			...
			query_incoming = "\
				...
				FROM\
					(\
					(SELECT\
						incoming0.location_dest_id as loc_id,\
						incoming0.product_id,\
						sum(incoming0.qty_kg) as qty,\
						sum(incoming0.product_uop_qty) as product_uop_qty,\
						incoming0.tracking_id,\
						'"+date_start+"' as date\
					FROM\
						(\
						SELECT\
							incoming0_a.location_dest_id, incoming0_a.product_id, incoming0_a.product_uop,\
							round(round(incoming0_a.product_qty/incoming0_d.factor,4)*incoming0_e.factor,4) as qty_kg,\
							incoming0_a.product_uop_qty, coalesce(incoming0_a.tracking_id,0) as tracking_id, to_char(incoming0_a.date,'YYYY-MM-DD') as sdate\
						FROM\
							stock_move incoming0_a\
							left join product_product incoming0_b on incoming0_b.id = incoming0_a.product_id\
							left join product_template incoming0_c on incoming0_c.id = incoming0_b.product_tmpl_id\
							left join product_uom incoming0_d on incoming0_d.id = incoming0_a.product_uom\
							left join product_uom incoming0_e on incoming0_e.id = incoming0_c.uom_id\
						WHERE\
							incoming0_a.location_dest_id = any(array"+str(loc_ids)+") and incoming0_a.location_id = any(array"+str(prod_loc_ids+supp_loc_ids+cust_loc_ids+int_loc_ids+adj_loc_ids)+")\
							and incoming0_a.product_id=any(array"+str(product_ids)+") and incoming0_a.state in ('done')\
							and incoming0_a.date < '"+date_start+"'\
						) incoming0\
					GROUP BY\
						loc_id,\
						incoming0.product_id,\
					...
					INNER JOIN\
						(\
						(SELECT\
							incoming_cum0.location_dest_id as loc_id,\
							incoming_cum0.product_id,\
							sum(incoming_cum0.qty_kg) as qty,\
							sum(incoming_cum0.product_uop_qty) as product_uop_qty,\
							incoming_cum0.tracking_id,\
							'"+date_start+"' as date\
						FROM\
							(\
							SELECT\
								incoming_cum0_a.location_dest_id, incoming_cum0_a.product_id, incoming_cum0_a.product_uop,\
								round(round(incoming_cum0_a.product_qty/incoming_cum0_d.factor,4)*incoming_cum0_e.factor,4) as qty_kg,\
								incoming_cum0_a.product_uop_qty, coalesce(incoming_cum0_a.tracking_id,0) as tracking_id, to_char(incoming_cum0_a.date,'YYYY-MM-DD') as sdate\
							FROM\
								stock_move incoming_cum0_a\
								left join product_product incoming_cum0_b on incoming_cum0_b.id = incoming_cum0_a.product_id\
								left join product_template incoming_cum0_c on incoming_cum0_c.id = incoming_cum0_b.product_tmpl_id\
								left join product_uom incoming_cum0_d on incoming_cum0_d.id = incoming_cum0_a.product_uom\
								left join product_uom incoming_cum0_e on incoming_cum0_e.id = incoming_cum0_c.uom_id\
							WHERE\
								incoming_cum0_a.location_dest_id = any(array"+str(loc_ids)+") and incoming_cum0_a.location_id = any(array"+str(prod_loc_ids+supp_loc_ids+cust_loc_ids+int_loc_ids+adj_loc_ids)+")\
								and incoming_cum0_a.product_id=any(array"+str(product_ids)+") and incoming_cum0_a.state in ('done')\
								and incoming_cum0_a.date < '"+date_start+"'\
							) incoming_cum0\
						GROUP BY\
							loc_id,\
							incoming_cum0.product_id,\
			...
			query_outgoing = "\
				...
				"
						# LINE 289: and incoming_a.date >= '"+date_start+"' \
...
class stock_ageing_parser_xls(report_xls):
	no_ind = 0
	...
	def generate_xls_report(self, parser, data, obj, wb):
		...
		for inventory_type in parser._get_inventory_type(data): 
			...
			if inventory_type.code == 'Finish':
				...
				rowcount+=1
				ws.write_merge(rowcount,rowcount,0,12, "Stock Ageing Report - %s"%inventory_type.name, title_style)
				rowcount+=1
				ws.write_merge(rowcount,rowcount,0,12, "AS ON : %s"%parser._get_date(data), title_style)
				rowcount+=2

				ws.write_merge(rowcount,rowcount+2,0,0, "Item Code", th_both_style)
				...
				ws.write_merge(rowcount,rowcount+1,3,4, "Qty As On\n"+datetime.strptime(data['as_on'],'%Y-%m-%d %H:%M:%S').strftime('%d/%m/%Y'), th_top_style)
				# ws.write_merge(rowcount, rowcount, 5, 12, "Age", th_top_style)
				period_1_to = datetime.strptime(data['as_on'],'%Y-%m-%d %H:%M:%S').strftime('%d/%m/%Y')
				period_1_from = datetime.strptime(data['as_on'],'%Y-%m-%d %H:%M:%S').strftime('01/%m/%Y')
				ws.write_merge(rowcount,rowcount+1, 5, 6, "Qty from "+period_1_from+"\nto "+period_1_to, th_both_style)
				
				period_2_to = (datetime.strptime(period_1_from,"%d/%m/%Y") + relativedelta(days=-1)).strftime("%d/%m/%Y")
				period_2_from = time.strftime("01/01/"+str(datetime.strptime(period_2_to,'%d/%m/%Y').year))
				ws.write_merge(rowcount,rowcount+1, 7, 8, "Qty from "+period_2_from+"\nto "+period_2_to, th_both_style)
				
				period_3_to = (datetime.strptime(period_2_from,'%d/%m/%Y') + relativedelta(days=-1)).strftime("%d/%m/%Y")
				period_3_from = time.strftime("01/01/"+str(datetime.strptime(period_3_to,'%d/%m/%Y').year))
				ws.write_merge(rowcount,rowcount+1, 9, 10, "Qty from "+period_3_from+"\nto "+period_3_to, th_both_style)
				
				ws.write_merge(rowcount,rowcount+1, 11, 12, "Qty Before\n"+period_3_from, th_both_style)
									...
									for uop_id in sorted(stock_lines_aged[parent_loc][loc_id][blend][product][tracking].keys()):
										for uom_id in sorted(stock_lines_aged[parent_loc][loc_id][blend][product][tracking][uop_id].keys()):
											for line in sorted(stock_lines_aged[parent_loc][loc_id][blend][product][tracking][uop_id][uom_id]):
												ws.write(rowcount, 0, prods[product]['code'], normal_style)
												ws.write(rowcount, 1, prods[product]['name'], normal_style)
												ws.write(rowcount, 2, '', normal_style)
												ws.write(rowcount, 3, line['uom_quantity'] or '', normal_style_float)
												ws.write(rowcount, 4, line['amount_value'] or '', normal_style_float)
												ws.write(rowcount, 5, line['qty1'] or '', normal_style_float)
												ws.write(rowcount, 6, line['amount_value1'] or '', normal_style_float)
												ws.write(rowcount, 7, line['qty2'] or '', normal_style_float)
												ws.write(rowcount, 8, line['amount_value2'] or '', normal_style_float)
												ws.write(rowcount, 9, line['qty3'] or '', normal_style_float)
												ws.write(rowcount, 10, line['amount_value3'] or '', normal_style_float)
												ws.write(rowcount, 11, line['qty4'] or '', normal_style_float)
												ws.write(rowcount, 12, line['amount_value4'] or '', normal_style_float)
												...
												rowcount+=1
							ws.write_merge(rowcount,rowcount,0,2, 'Subtotal : '+str(blend), subtotal_style)
							for c in range(3,13):
								ws.write(rowcount,c,subtotal_blend[c] or '', subtotal_style2)
...
report_sxw.report_sxw('report.stock.ageing.report.pdf','stock.ageing.wizard', 'addons/ad_stock_report/report/stock_ageing_report.mako',
							parser=stock_ageing_parser)
stock_ageing_parser_xls('report.stock.ageing.report.xls','stock.ageing.wizard', 'addons/ad_stock_report/report/stock_ageing_report.mako',
						parser=stock_ageing_parser)