...
class summary_stock_parser_xls(report_xls):
	...
	def generate_xls_report(self, parser, data, obj, wb):
		...
		for inventory_type in parser._get_inventory_type(data): 
			...
			ws.write(rowcount,6, "Mr. Shah & Mr. Shailendra")
			rowcount+=1
			...
			for parent_loc in sorted(stock_summary.keys()):
				ws.write_merge(rowcount,rowcount,0,5,parent_loc,normal_bold_style_b)
				rowcount+=1
				for line in stock_summary[parent_loc]:
					...
					rounder=line.get('product_uop_qty_pres',False) and round(line.get('product_uop_qty_pres',0.0)) or 0
					ws.write(rowcount,2, rounder !=0 and line.get('product_uop_qty_pres',0.0) or 0, normal_style_float_round)
					
					rounder=line.get('qty_kg_pres',False) and round(line.get('qty_kg_pres',0.0),2) or 0.0
					ws.write(rowcount,3, rounder !=0.0 and line.get('qty_kg_pres',0.0) or 0.0, normal_style_float)
					
					rounder=line.get('qty_bale_pres',False) and round(line.get('qty_bale_pres',0.0),2) or 0.0
					ws.write(rowcount,4, rounder !=0.0 and line.get('qty_bale_pres',0.0) or 0.0, normal_style_float)
					
					rounder=line.get('qty_bale_prev',False) and round(line.get('qty_bale_prev',0.0),2) or 0.0
					...
					total_parent.update({
						...
						2:total_parent[2]+(line.get('qty_kg_pres',False) and line.get('qty_kg_pres',0.0) or 0.0) ,
