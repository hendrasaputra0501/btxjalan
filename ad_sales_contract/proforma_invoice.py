...
class proforma_invoice(osv.osv):
	...
	def default_get(self, cr, uid, fields, context):
		...
		res = super(proforma_invoice, self).default_get(cr, uid, fields, context=context)
	...
	def create(self, cr, uid, vals, context=None):
		res=super(proforma_invoice, self).create(cr, uid, vals, context=context)
		...
		if pi_id:
			...
			if pi_id.sale_id:
				if pi_id.sale_id.sale_type=='export':
					...
					self.write(cr, uid, res ,{'name': (company_code+'L '+(self.pool.get('ir.sequence').get(cr, uid, 'proforma.invoice.local') or '/'))})
			else:
				self.write(cr, uid, res ,{'name': (company_code+' '+(self.pool.get('ir.sequence').get(cr, uid, 'proforma.invoice') or '/'))})		
		return res
...
class proforma_invoice_line(osv.osv):
	...
	_columns = {
		...
		'origin': fields.char('Source Document', size=256, help="Reference of the document that produced this proforma invoice."),
		'sequence': fields.integer('Sequence', help="Gives the sequence of this line when displaying the invoice."),
		'invoice_id': fields.many2one('proforma.invoice', 'Proforma Invoice', ondelete='cascade', select=True),
		...
		'invoice_line_tax_id': fields.many2many('account.tax', 'proforma_invoice_line_tax', 'proforma_invoice_line_id', 'tax_id', 'Taxes', domain=[('parent_id','=',False)]),
