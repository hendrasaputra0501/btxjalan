...
class DetailOutstandingFreightCost(report_sxw.rml_parse):
	...
	def _get_freight_from_invoice(self,pick,last_pay_date):
		cr = self.cr
		uid = self.uid
				...
				if charge.type_of_charge.id == charge_type_id:
					if charge.picking_related_id and charge.picking_related_id.id == pick.id and charge.report_charge_type=='freight':
						...
						if charge.currency_id and charge.company_id and charge.company_id.tax_base_currency and charge.currency_id.id == charge.company_id.tax_base_currency.id:
							...
							ch = self.pool.get('res.currency').compute(cr,uid,charge.currency_id.id,charge.company_id.currency_id.id,cost['total_included'],context=ctx)
							inv_price = self.pool.get('res.currency').compute(cr,uid,charge.currency_id.id,charge.company_id.currency_id.id,charge.invoice_id.amount_total,context=ctx)
							inv_bal = self.pool.get('res.currency').compute(cr,uid,charge.currency_id.id,charge.company_id.currency_id.id,self._get_invoice_balance_outstanding(charge.invoice_id,last_pay_date),context=ctx)
