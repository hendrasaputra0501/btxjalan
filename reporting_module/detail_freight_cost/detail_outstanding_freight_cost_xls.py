...
from detail_outstanding_freight_cost_parser import *
from collections import OrderedDict

class detail_outstanding_freight_cost_xls(report_xls):
	no_ind = 0
	...
	def generate_xls_report(self, parser, data, obj, wb):
		...
		for curr in currencies:
			...
			ws.write_merge(1,1,0,18, "DETAIL OF OUTSTANDING OCEAN FREIGHT COST", title_style)
			ws.write_merge(2,2,0,18, "AS ON: %s"%(date_range[1]),title_style)
			...
			ws.write_merge(4,5,18,18, "Total EMKL", th_both_style)
			ws.write_merge(4,5,19,19, "Total", th_both_style)
			...
			max_width_col[9] = len("freight us")
			max_width_col[10] = 9
			max_width_col[11] = 12
			max_width_col[12] = 8
			max_width_col[13] = 12
			...
			for pick in parser._get_result(data,curr.id):
				...
				#cols[9]=(parser._get_freight_from_invoice(pick,date_range[1],curr.id) + parser._get_fob_from_invoice(pick,date_range[1],curr.id)) or ""
				cols9_1=parser._get_freight_from_invoice(pick,date_range[1],curr.id)
				cols9_2=parser._get_fob_from_invoice(pick,date_range[1],curr.id)
				if (cols9_1=="Paid") and (cols9_2==False):					
					cols[9]="Paid"
				elif (cols9_1==False) and (cols9_2=="Paid"):					
					cols[9]="Paid"
				else:
					cols[9]=0.0
					if cols9_1 not in ["Paid","",False]:
						cols[9]+=cols9_1
					if cols9_2 not in ["Paid","",False]:
						cols[9]+=cols9_2
				cols[99]=parser._get_freight_from_invoice(pick,date_range[1],pick.company_id.currency_id.id) or ""
				...
				cols[11]=pick.trucking_charge and parser._get_emkl_from_invoice(pick,date_range[1]) or ""
				cols[12]=pick.lifton_bpa_id and parser._get_lifton_bpa(pick,pick.company_id.currency_id.id,date_range[1]) or ''
				cols[13]=pick.lifton_bpa_id and parser._get_lifton_bpa(pick,pick.company_id.tax_base_currency.id,date_range[1]) or '' 
				cols[14]=pick.invoice_id and parser._get_other_cost_from_invoice(pick,pick.company_id.currency_id.id,date_range[1]) or ''
				cols[15]=pick.invoice_id and parser._get_other_cost_from_invoice(pick,pick.company_id.tax_base_currency.id,date_range[1]) or ''
				# fs ="K"+str(current_row+1)+"+P"+str(current_row+1)
				cols[16]=(type(cols[99])==float and cols[99] or 0.0) + (type(cols[14])==float and cols[14] or 0.0) +parser._convert_rate(cols[15],pick.invoice_id.date_invoice,pick)
				cols[17]=parser._get_fob_from_invoice(pick,date_range[1],pick.company_id.currency_id.id) or 0.0
				cols[18]=parser._convert_rate(cols[11] or 0.0,pick.invoice_id.date_invoice,pick) + (type(cols[12])==float and cols[12] or 0.0) + parser._convert_rate(cols[13],pick.invoice_id.date_invoice,pick) 
				#cols[19]=cols[16]+cols[17]+cols[18]
				if ((cols[16]=="Paid") or (cols[17]=="Paid") or (cols[18]=="Paid")) and (cols[16] in ["Paid","",False]) and (cols[17] in ["Paid","",False]) and (cols[18] in ["Paid","",False]):
					cols[19]="Paid"
				else:
					cols[19]=0.0
					if cols[16] not in ["Paid","",False]:
						cols[19]+=cols[16]
					if cols[17] not in ["Paid","",False]:
						cols[19]+=cols[17]
					if cols[18] not in ["Paid","",False]:
						cols[19]+=cols[18]
				
				# if pick.name == "BES-116-0175":
				# 	print "################################continue#################################",cols[9], cols[11], cols[12], cols[13], cols[14], cols[15]
				if (cols[9] in ["Paid","",False]) and (cols[11] in ["Paid","",False]) and (cols[12] in ["Paid","",False]) and (cols[13] in ["Paid","",False]) and (cols[14] in ["Paid","",False]) and (cols[15] in ["Paid","",False]):
					continue
				if cols[9] and cols[9]!="" and cols[9]!='Paid' and cols[8] not in freight_cost.keys():
					freight_cost.update({cols[8]:cols[9] or 0.0})
					freight_cost_2.update({cols[8]:cols[99] or 0.0})
				elif cols[9] and cols[9]!="" and cols[9]!='Paid' and cols[8] in freight_cost.keys():
					freight_cost.update({cols[8]:freight_cost[cols[8]]+(cols[9] or 0.0)})
					freight_cost_2.update({cols[8]:freight_cost_2[cols[8]]+(cols[99] or 0.0)})

				if cols[11] and cols[11]!="" and cols[11]!='Paid' and cols[10] not in emkl_cost.keys():
					emkl_cost.update({cols[10]:cols[11] or 0.0})
				elif cols[11] and cols[11]!="" and cols[11]!='Paid' and cols[10] in emkl_cost.keys():
					emkl_cost.update({cols[10]:emkl_cost[cols[10]]+(cols[11] or 0.0)})

				if ((cols[12] and cols[12]!="" and cols[12]!='Paid') or (cols[13] and cols[13]!="" and cols[13]!='Paid')) and cols[10] not in other_emkl_cost.keys():
					other_emkl_cost.update({cols[10]:[
						(cols[12]!='Paid' and cols[12] or 0.0),(cols[13]!='Paid' and cols[13] or 0.0)]})
				elif ((cols[12] and cols[12]!="" and cols[12]!='Paid') or (cols[13] and cols[13]!="" and cols[13]!='Paid')) and cols[10] in other_emkl_cost.keys():
					other_emkl_cost.update({
						cols[10]:[other_emkl_cost[cols[10]][0]+(cols[12]!='Paid' and cols[12] or 0.0),other_emkl_cost[cols[10]][1]+(cols[13]!='Paid' and cols[13] or 0.0) ]
						})

				if ((cols[14] and cols[14]!="" and cols[14]!='Paid') or (cols[15] and cols[15]!="" and cols[15]!='Paid')) and cols[8] not in other_cost.keys():
					other_cost.update({cols[8]:[(cols[14]!='Paid' and cols[14] or 0.0),(cols[15]!='Paid' and cols[15] or 0.0)]})
				elif ((cols[14] and cols[14]!="" and cols[14]!='Paid') or (cols[15] and cols[15]!="" and cols[15]!='Paid')) and cols[8] in other_cost.keys():
					other_cost.update({
						cols[8]:[other_cost[cols[8]][0]+(cols[14]!='Paid' and cols[14] or 0.0),other_cost[cols[8]][1]+(cols[15]!='Paid' and cols[15] or 0.0)]
						})
			...
			for z in [0,2,6,7,8,16]:
			 	ws.col(z).width = int(256*max_width_col[z]*1.1)
		pass

#from netsvc import Service
#del Service._services['report.stock.report.bitratex']
detail_outstanding_freight_cost_xls('report.detail.outstanding.freight.cost.report','detail.freight.cost', 'addons/reporting_module/detail_freight_cost/detail_freight_cost.mako',
						parser=DetailOutstandingFreightCost)
