		...
		if picking_ids:
			...
			for pick in res:
				...
				if pick and pick.invoice_id and pick.invoice_id.charge_invoice_ids:
					...
					if pick.forwading and pick.forwading.id and pick.forwading_charge:
				...
				if pick.id==7211:
					print "=-=======================",currency_id,return_values,found

		return return_values

	def _get_freight_from_invoice(self,pick,currency_id):
		cr = self.cr
		uid = self.uid
		...
		if pick and pick.invoice_id and pick.invoice_id.charge_invoice_ids:
				...
				if charge.type_of_charge.id == charge_type_id:
					...
					if charge.picking_related_id and charge.picking_related_id.id == pick.id and charge.report_charge_type=='freight':
						...
						if charge.currency_id and charge.company_id and charge.company_id.tax_base_currency and charge.currency_id.id == charge.company_id.tax_base_currency.id:
							...
							total_charge_usd += self.pool.get('res.currency').compute(cr,uid,charge.currency_id.id,currency_id,cost['total_included'],context=ctx)
			...
			if pick.forwading and pick.forwading.id:
				...
				total_charge_usd = 0.0
				# if charge.currency_id.id == currency_id:
				# 	total_charge_usd = charge.cost
				# else:
				# 	ctx={'date':pick.invoice_id.date_invoice or pick.date_done or False}
				# 	total_charge_usd = self.pool.get('res.currency').compute(cr,uid,charge.currency_id.id,currency_id,charge.cost,context=ctx)
			
		return total_charge_usd

	def _get_fob_from_invoice(self,pick,currency_id):
		cr = self.cr
		uid = self.uid
		total_charge_usd = 0.0
		charge_type_id = self.pool.get('charge.type').search(cr,uid,[('name','=','Freight')])
