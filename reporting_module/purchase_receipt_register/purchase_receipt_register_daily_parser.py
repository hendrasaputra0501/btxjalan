...
class PurchaseReceiptRegisterDaily (report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context=None):
		super(PurchaseReceiptRegisterDaily, self).__init__(cr, uid, name, context=context)
	...
	def _get_result(self, data):
		...
		query = "\
			...
			coalesce(discount.discount_amt, 0)/100 as discount,\
				...
				i.name as prod_name, \
				a.product_uop_qty as qty2, \
				m.name as uom_name, \
				n.name as lot_number, \
				coalesce(a.net_weight,0) as nw, \
				a.moisturity as mois, a.product_qty as qty1, \
			...
			FROM \
				...
				LEFT JOIN (\
					select distinct mr.name as mr_name,material_req_line_id,mrl.product_id,po.id as order_id from purchase_requisition_line prl \
										left join material_request_line mrl on prl.material_req_line_id=mrl.id \
										left join material_request mr on mrl.requisition_id=mr.id \
										left join purchase_requisition pr ON pr.id=prl.requisition_id \
										left join purchase_order po ON po.requisition_id=pr.id \
			...
			WHERE \
				a.state='done' AND b.type='in' \
				AND (c.goods_type='%s' or b.goods_type='%s') \
				AND (b.date_done::date>='%s' and b.date_done::date<='%s')\
				AND b.purchase_type in %s "+force_picking_conditions+"\
			GROUP BY a.id, a.price_unit, a.product_qty, b.id, b.name, to_char(b.date_done,'DD/MM/YY'),to_char(b.date_done,'YYYY-MM-DD'), c.name,to_char(c.date_order,'DD/MM/YY'),d.id,d.partner_code,d.name,e.name,b.supplier_delicery_slip,to_char(b.date_delivery_slip,'DD/MM/YY'),f.default_code,i.name,a.product_uop_qty,m.name,n.name,coalesce(a.net_weight,0),a.moisturity,h.name,po_price_unit,j.reference,coalesce(k.price_unit,0),j.number,b.contract_purchase_number,to_char(j.date_due,'DD/MM/YY'),l.name,coalesce(l.alias,l.name), coalesce(o.part_number,''),discount.discount_amt,h.id,q.currency_id,c.date_order\
			ORDER BY b.id\
			"
		query = query%(goods_type,goods_type,date_start,date_end,purchase_type)
	...
	def _get_difference(self,date_start,date_end):
		...
		return ""
report_sxw.report_sxw('report.purchase.receipt.register.daily.report','purchase.receipt.register.daily.wizard', 'addons/reporting_module/purchase_receipt_register/purchase_receipt_register_daily.mako', parser=PurchaseReceiptRegisterDaily)
...
class purchase_receipt_register_wizard_xls(report_xls):
	...
	def generate_xls_report(self, parser, data, obj, wb):
		...
		ws.write_merge(1,1,0, add_info and 28 or 20, "PURCHASE RECEIPT RECORD", title_style)
		...
		ws.write(5,18, "COST", th_bottom_style)
		...
		for name in sorted(result_grouped.keys(),key = lambda x: (rm_goods_type and data['form']['header_group_by']=='vendor_wise' and x[1] or x)):
			...
			for o in sorted(result_grouped[name], key = lambda x : (x['mrr_date2'],x['mrr_no'])):       
				...
				ws.write(rowcount,12,o['qty2'],normal_style_float)
				ws.write(rowcount,13,o['nw'],normal_style_float)
				ws.write(rowcount,14,o['mois'],normal_style_float)
				ws.write(rowcount,15,o['qty1'],normal_style_float)
				ws.write(rowcount,16,o['curr_name'],normal_style)
				ws.write(rowcount,17,o['po_price_unit']*o['qty1'],normal_style_float)
				ws.write(rowcount,18,o['cost'],normal_style_float)
				ws.write(rowcount,19,o['cost']*o['qty1'],normal_style_float)
				...
				if add_info:
					...
					ws.write(rowcount,22,o['inv_cost'],normal_style)
					ws.write(rowcount,23,o['inv_cost']*o['qty1'],normal_style)
					...
					ws.write(rowcount,28,(o['inv_cost'] and o['inv_cost']-o['po_price_unit'] or 0.0),normal_style)

				st=st+o['qty2']
				st1=st1+o['nw']
				st2=st2+o['qty1']
				st3=st3+o['po_price_unit']*o['qty1']
				st4=st4+(o['cost']*o['qty1'])
				rowcount+=1
...
purchase_receipt_register_wizard_xls('report.xls.purchase.receipt.register.daily.report','purchase.receipt.register.daily.wizard', 'addons/reporting_module/purchase_receipt_register/purchase_receipt_register_daily.mako',
						parser=PurchaseReceiptRegisterDaily)
