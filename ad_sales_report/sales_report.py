...
class report_sales(osv.osv_memory):
    _name = "report.sales.wizard"
    _columns = {
            "report_type"       : fields.selection([('product','Product Wise'),('customer','Customer Wise'),('date','Date Wise'),('invoice','Invoice Wise'),('country','Country Wise'),('raw_format','Raw Detail')],'Report Type',required=True),
            "goods_type"        : fields.selection([('finish','Finish Goods'),('finish_others','Finish Goods(Others)'),('raw','Raw Material'),('service','Services'),('waste','Waste'),('scrap','Scrap'),('asset','Fixed Asset')],'Goods Type',required=True),
    ...
    def compute_report(self, cr, uid, ids, context={}):
        ...
        if data['report_type']!='raw_format':
            return {
