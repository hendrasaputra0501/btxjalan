		...
		cr.execute("\
			...
			BEGIN \
				RETURN QUERY \
					...
					chg1 as ( \
						select invoice_id,picking_related_id,invoice_related_id,currency_id, \
						...
						group by invoice_id,picking_related_id,invoice_related_id,currency_id,chg1_rate \
						), \
					chg2 as ( \
					...
					chg4 as ( \
						...
						round((1/coalesce(rcrinv_in1.rate,1))*ail2.price_subtotal,2) as chg4_subtotal_usd, \
					...
					from ( \
						...
						sol.sequence_line, \
						rp.partner_alias,pp.id as prod_id,pp.name_template as product_name,ail.price_subtotal_usd,rp_soa.name as agent, rp_soa.id as agent_id, \
						...
						coalesce(round((round(sm.product_qty/pu1.factor*pu2.factor,4)/sm_ids.total_qty_do)*chg2.chg2_subtotal_usd,2),0.0)+ \
						coalesce(round((round(sm.product_qty/pu1.factor*pu2.factor,4)/sm_ids.total_qty_do)*chg3.chg3_subtotal_usd,2),0.0)+ \
						...
						coalesce(round((round(sm.product_qty/pu1.factor*pu2.factor,4)/sm_ids.total_qty_do)*chg3.chg3_subtotal_usd,2),0.0)+ \
						coalesce(round((round(sm.product_qty/pu1.factor*pu2.factor,4)/sm_ids.total_qty_do)*chg4.chg4_subtotal_usd,2),0.0)) \
						- round((round(sm.product_qty/pu1.factor*pu2.factor,4)/sm_ids2.total_qty_do)*ail.amount_insurance_usd,2) \
						) as amount_fob, \
						...
						group by slprod2.name,ail.invoice_id,ail.internal_number,ail.date_invoice,lc.name,sp.name,sp.date_done::date,sol.sequence_line,rp.partner_alias, \
							...
							chg2.chg2_subtotal_usd,chg3.chg3_subtotal_usd,chg4.chg4_subtotal_usd,rp_soa.name,rp_soa.id,soa.commission_percentage,inco.code, \
