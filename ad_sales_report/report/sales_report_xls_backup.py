        ...
        for sheet in sheets:
            ...
            if data['form']['sale_type'] == 'export':
                ...
                cols_specs = [
                        ...
                        ('headerTotAmount',  1, 0, 'text', lambda x, d, p: 'Total Amount'),
                        ('headerTerms',  1, 0, 'text', lambda x, d, p: 'Terms'),
                ...
                cols_specs = [
                        ...
                        ('headerContractNo',  1, 0, 'text', lambda x, d, p: 'Contract No'),
                        ('headerUOMQty',  1, 0, 'text', lambda x, d, p: 'Qty (' + uom_base + ')'),
                        ('headerNetPrice',  1, 0, 'text', lambda x, d, p: 'Net Price (' + price_base + ')'),
                        ('headerTax',  1, 0, 'text', lambda x, d, p: 'Tax'),
                        ('headerSellPrice',  1, 0, 'text', lambda x, d, p: 'Sell Price (' + price_base + ')'),
                        ('headerNetAmount',  1, 0, 'text', lambda x, d, p: 'Net Amount (' + amount_base + ')'),
                        ('headerTaxAmount',  1, 0, 'text', lambda x, d, p: 'Tax Amount (' + amount_base + ')'),
                        ('headerTotAmount',  1, 0, 'text', lambda x, d, p: 'Total Amount (' + amount_base + ')'),
                        ('headerTerms',  1, 0, 'text', lambda x, d, p: 'Terms'),
                        ('headerDPPNPET',  1, 0, 'text', lambda x, d, p: 'DPP N-PET'),
                        ('headerDPPPET',  1, 0, 'text', lambda x, d, p: 'DPP PET'),
                        ('headerTaxNPET',  1, 0, 'text', lambda x, d, p: 'Tax N-PET '),
            ...
            if data['form']['sale_type'] == 'export':
                if sheet == 'date':
                    row_header              = self.xls_row_template(cols_specs, ['headerSJNo','headerSJDate','headerInvoiceNo',
                                                                                 'headerCustomer','headerProductCode','headerProductName',
                ...
                row_header              = self.xls_row_template(cols_specs, ['headerSJNo','headerSJDate','headerTaxInvc',
                                                                             'headerCustomer','headerProduct','headerContractNo',
                                                                             'headerUOMQty','headerCuryId','headerNetPrice','headerTax',
                                                                             'headerSellPrice','headerNetAmount','headerTaxAmount',
                                                                             'headerTotAmount','headerTerms','headerDPPNPET',
                                                                             'headerDPPPET','headerTaxNPET','headerTaxPET'])
            else:
                row_header              = self.xls_row_template(cols_specs, ['headerSJNo','headerSJDate','headerTaxInvc',
                                                                             'headerCustomer','headerProduct','headerContractNo',
                                                                             'headerUOMQty','headerNetPrice','headerTax',
                                                                             'headerSellPrice','headerNetAmount','headerTaxAmount',
                                                                             'headerTotAmount','headerTerms','headerDPPNPET',
                                                                             'headerDPPPET','headerTaxNPET','headerTaxPET'])
            ...
            if data['form']['sale_type'] == 'export':
                ...
                wsa.col(7).width = width02*2
                wsa.col(8).width = (width02*3)/5
                wsa.col(9).width = width02*6/5
                ...
                wsa.col(16).width = (width02*4)/5
                wsa.col(17).width = (width02*3)/2
                wsa.col(18).width = (width02*3)/2
                wsa.col(19).width = (width02*3)/2
                wsa.col(20).width = width02/2
                col_max = 20  
            else:
                wsa.col(0).width = width01
                wsa.col(1).width = width01
                ...
                wsa.col(8).width = (width02*4)/5
                wsa.col(9).width = (width02*4)/5
                wsa.col(10).width = width02/2
                wsa.col(11).width = (width02*4)/5
                wsa.col(12).width = (width02*4)/5
                wsa.col(13).width = (width02*4)/5
                wsa.col(14).width = (width02*4)/5
                wsa.col(15).width = (width02*4)/5
                wsa.col(16).width = (width02*3)/2
                wsa.col(17).width = (width02*3)/2
                wsa.col(18).width = (width02*3)/2
                wsa.col(19).width = width02/2
                col_max = 19  
            ...
            for line in details:   
                ...
                if (loc_name != old_loc_name) or (group_name != old_group_name):
                    if data['form']['sale_type'] == 'export':
                        if old_group_name != 'None':
                            ...
                            wsa.write_merge(row_count,row_count,1,9, 'Total for ' + old_group_name, subtotal_title_style)        
                            wsa.write(row_count, 10, group_qty_bales, subtotal_style)
                            wsa.write(row_count, 11, group_qty_kgs, subtotal_style)
                            if sheet == 'date':
                                wsa.write_merge(row_count,row_count,12,15, '', subtotal_style)
                                if group_items > 0:    
                                    wsa.write(row_count, 16, group_kmk_rate/group_items, subtotal_style2)
                                else:
                                    wsa.write(row_count, 16, '', subtotal_style2)
                        ...
                        if old_group_name != 'None':
                            ...
                            wsa.write(row_count, 15, group_cury_tot_amount, subtotal_style2)
                            wsa.write_merge(row_count,row_count,16,16, '', subtotal_style)    
                            wsa.write(row_count, 17, group_dpp_npet, subtotal_style2)
                            wsa.write(row_count, 18, group_dpp_pet, subtotal_style2)
                            wsa.write(row_count, 19, group_tax_npet, subtotal_style2)
                            wsa.write(row_count, 20, group_tax_pet, subtotal_style2)
                            row_count+=1    
                            #wsa.write_merge(row_count,row_count,0,col_max, '', normal_style)
                            #row_count+=1
                    else:
                        if old_group_name != 'None':
                            ...
                            wsa.write(row_count, 12, group_net_amount, subtotal_style2)
                            wsa.write(row_count, 13, group_tax_amount, subtotal_style2)
                            wsa.write(row_count, 14, group_tot_amount, subtotal_style2)
                            wsa.write_merge(row_count,row_count,15,15, '', subtotal_style)    
                            wsa.write(row_count, 16, group_dpp_npet, subtotal_style2)
                            wsa.write(row_count, 17, group_dpp_pet, subtotal_style2)
                            wsa.write(row_count, 18, group_tax_npet, subtotal_style2)
                            wsa.write(row_count, 19, group_tax_pet, subtotal_style2)
                            row_count+=1    
                            #wsa.write_merge(row_count,row_count,0,col_max, '', normal_style)
                            #row_count+=1
                    ...
                    if (loc_name != old_loc_name):
                        if data['form']['sale_type'] == 'export':
                            if old_loc_name != 'None':
                                wsa.write_merge(row_count,row_count,0,9, 'Total for ' + old_loc_name, subtotal_title_style)        
                                wsa.write(row_count, 10, loc_qty_bales, subtotal_style)
                                wsa.write(row_count, 11, loc_qty_kgs, subtotal_style)
                                if sheet == 'date':
                                    wsa.write_merge(row_count,row_count,12,16, '', subtotal_style)    
                            ...
                            if old_loc_name != 'None':
                                ...
                                wsa.write(row_count, 15, loc_cury_tot_amount, subtotal_style2)
                                wsa.write_merge(row_count,row_count,16,16, '', subtotal_style)    
                                wsa.write(row_count, 17, loc_dpp_npet, subtotal_style2)
                                wsa.write(row_count, 18, loc_dpp_pet, subtotal_style2)
                                wsa.write(row_count, 19, loc_tax_npet, subtotal_style2)
                                wsa.write(row_count, 20, loc_tax_pet, subtotal_style2)
                                row_count+=1    
                            ...
                            if old_loc_name != 'None':
                                ...
                                wsa.write(row_count, 8, loc_uom_qty, subtotal_style)
                                wsa.write_merge(row_count,row_count,9,11, '', subtotal_style)    
                                wsa.write(row_count, 12, loc_net_amount, subtotal_style2)
                                wsa.write(row_count, 13, loc_tax_amount, subtotal_style2)
                                wsa.write(row_count, 14, loc_tot_amount, subtotal_style2)
                                wsa.write_merge(row_count,row_count,15,15, '', subtotal_style)    
                                wsa.write(row_count, 16, loc_dpp_npet, subtotal_style2)
                                wsa.write(row_count, 17, loc_dpp_pet, subtotal_style2)
                                wsa.write(row_count, 18, loc_tax_npet, subtotal_style2)
                                wsa.write(row_count, 19, loc_tax_pet, subtotal_style2)
                                row_count+=1    
                                wsa.write_merge(row_count,row_count,0,19, '', normal_style)
                                row_count+=1
                ...
                if qty_bales>0.0:
                    sell_price_idr_bales = (line['tot_amount_idr'] or 0.0)/qty_bales
                else:
                ...
                if data['form']['sale_type'] == 'export':
                    ...
                    wsa.write(row_count, 7, line['prod_name'] or '', normal_style)
                    wsa.write(row_count, 8, line['tracking_name'] or '', normal_style)
                    wsa.write(row_count, 9, line['contract_no'] or '', normal_style)
                    wsa.write(row_count, 10, qty_bales, normal_right_style)
                    wsa.write(row_count, 11, qty_kgs, normal_right_style)
                    wsa.write(row_count, 12, line['curr_name'] or '', normal_style)
                    wsa.write(row_count, 13, sell_price_idr_bales, normal_right_style2)
                    wsa.write(row_count, 14, sell_price_usd_kgs, normal_right_style2)
                    ...
                    wsa.write(row_count, 8, base_uom_qty, normal_right_style)
                    ...
                    wsa.write(row_count, 15, cury_tot_amount, normal_right_style2)
                    wsa.write(row_count, 16, line['payment_term_name'] or '', normal_style)
                    wsa.write(row_count, 17, dpp_npet, normal_right_style2)
                    wsa.write(row_count, 18, dpp_pet, normal_right_style2)
                    wsa.write(row_count, 19, tax_npet, normal_right_style2)
                    wsa.write(row_count, 20, tax_pet, normal_right_style2)
                else:
                    wsa.write(row_count, 0, '', normal_style)
                    wsa.write(row_count, 1, '', normal_style)
                    wsa.write(row_count, 2, line['do_name'] or '', normal_style)
                    ...
                    wsa.write(row_count, 7, line['contract_no'] or '', normal_style)
                    wsa.write(row_count, 8, base_uom_qty, normal_right_style)
                    wsa.write(row_count, 9, base_net_price, normal_right_style2)
                    wsa.write(row_count, 10, line['tax_percent'] or 0.0, normal_right_style2)
                    wsa.write(row_count, 11, base_sell_price, normal_right_style2)
                    wsa.write(row_count, 12, net_amount, normal_right_style2)
                    wsa.write(row_count, 13, tax_amount, normal_right_style2)
                    wsa.write(row_count, 14, tot_amount, normal_right_style2)
                    wsa.write(row_count, 15, line['payment_term_name'] or '', normal_style)
                    wsa.write(row_count, 16, dpp_npet, normal_right_style2)
                    wsa.write(row_count, 17, dpp_pet, normal_right_style2)
                    wsa.write(row_count, 18, tax_npet, normal_right_style2)
                    wsa.write(row_count, 19, tax_pet, normal_right_style2)
                row_count+=1
            ...
            if data['form']['sale_type'] == 'export':
                ...
                wsa.write(row_count, 10, grand_qty_bales, total_style)
                wsa.write(row_count, 11, grand_qty_kgs, total_style)
                if sheet == 'date':
                    wsa.write_merge(row_count,row_count,12,16, '', total_style)    
                    wsa.write(row_count, 17, grand_tot_amount_usd, total_style2)
                    wsa.write(row_count, 18, grand_tot_amount_idr, total_style2)
                else:
                    wsa.write_merge(row_count,row_count,12,15, '', total_style)    
                ...
                wsa.write(row_count, 15, grand_cury_tot_amount, total_style2)
                wsa.write_merge(row_count,row_count,16,16, '', total_style)    
                wsa.write(row_count, 17, grand_dpp_npet, total_style2)
                wsa.write(row_count, 18, grand_dpp_pet, total_style2)
                wsa.write(row_count, 19, grand_tax_npet, total_style2)
                wsa.write(row_count, 20, grand_tax_pet, total_style2)
            else:
                if old_loc_name != 'None':
                    wsa.write_merge(row_count,row_count,0,7, 'Total for ' + old_loc_name, subtotal_title_style)        
                    wsa.write(row_count, 8, loc_uom_qty, subtotal_style)
                ...
                wsa.write(row_count, 12, grand_net_amount, total_style2)
                wsa.write(row_count, 13, grand_tax_amount, total_style2)
                wsa.write(row_count, 14, grand_tot_amount, total_style2)
                wsa.write_merge(row_count,row_count,15,15, '', total_style)    
                wsa.write(row_count, 16, grand_dpp_npet, total_style2)
                wsa.write(row_count, 17, grand_dpp_pet, total_style2)
                wsa.write(row_count, 18, grand_tax_npet, total_style2)
                wsa.write(row_count, 19, grand_tax_pet, total_style2)
