...
class ReportSalesSummary(report_sxw.rml_parse):
    ...
    def _get_title(self,data,sheet):
        if data['form']['sale_type'] == 'export':
            ...
            stitle = ''
        stitle = stitle + 'SALES SUMMARY STATEMENT - '
        if sheet == 'customer':
    ...
    def _get_view(self,data,sheet,context=None):
        if sheet == 'customer':
            s = "select substring(coalesce(u.alias,u.name,''),1,3) as dept_id, \
                ...
                sum(-g.product_uom_qty*coalesce(ap.factor,0.0)/coalesce(f.factor,1.0)) as qty_kg, \
                sum(-g.product_uom_qty*coalesce(aq.factor,0.0)/coalesce(f.factor,1.0)) as qty_bale, \
                ...
                sum(case when a.sale_type = 'local' and a.locale_sale_type = 'okb' then \
                    ...
                    end) as gross_amount_cury, \
                sum(coalesce(x.price_subtotal,0.0) * \
                    case when coalesce(z2.name,'') = 'IDR' then \
                        case when coalesce(z.name,'') = 'IDR' then 1.0 / (coalesce(ak.rate,1.0) * coalesce(ai.rate,0.0)) \
                        else 1.0 / coalesce(ab.rate,1.0) end \
                    else 1.0 / coalesce(ab.rate,1.0) end) as net_amount, \
                sum(coalesce(x.price_subtotal,0.0) * coalesce(ai.rate,0.0)) as net_amount_usd, \
                sum(coalesce(x.price_subtotal,0.0)) as net_amount_cury \
                from (  select g1.sale_id,g1.name,g1.date_done,g1.invoice_id,g2.sequence_line,g2.sale_line_id,g2.location_id, \
                        ...
                        group by g1.sale_id,g1.name,g1.date_done,g1.invoice_id,g2.sequence_line,g2.sale_line_id,g2.location_id, \
            ...
            if data['form']['sale_type'] == 'export':
                self.cr.execute (s + " where a.goods_type = %s \
                                       and a.sale_type = %s \
                                       group by substring(coalesce(u.alias,u.name,''),1,3),coalesce(d.partner_alias,d.name) \
            ...
            s = "select substring(coalesce(u.alias,u.name,''),1,3) as dept_id, \
                ...
                sum(case when a.sale_type = 'local' and a.locale_sale_type = 'okb' then \
                        coalesce(x.price_subtotal,0.0)+coalesce(x.tax_amount,0.0) \
                    else coalesce(x.price_subtotal,0.0) end) as gross_amount_cury, \
                sum(coalesce(x.price_subtotal,0.0) * \
                    case when coalesce(z2.name,'') = 'IDR' then \
                        case when coalesce(z.name,'') = 'IDR' then 1.0 / (coalesce(ak.rate,1.0) * coalesce(ai.rate,0.0)) \
                        else 1.0 / coalesce(ab.rate,1.0) end \
                ...
                sum(coalesce(x.price_subtotal,0.0) * \
                    ...
                    else coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end) as net_amount_usd, \
                sum(coalesce(x.price_subtotal,0.0)) as net_amount_cury \
                from (  select g1.sale_id,g1.name,g1.date_done,g1.invoice_id,g2.sequence_line,g2.sale_line_id,g2.location_id, \
                            g2.location_dest_id,g2.product_id,g2.tracking_id,g2.invoice_line_id, \
                            sum(case g1.type \
            ...
            if data['form']['sale_type'] == 'export':
                self.cr.execute (s + " where a.goods_type = %s \
                                       and a.sale_type = %s \
                                       group by substring(coalesce(u.alias,u.name,''),1,3),to_char(g.date_done,'DD/MM/YYYY') \
