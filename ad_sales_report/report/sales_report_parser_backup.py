...
class ReportSalesOrder(report_sxw.rml_parse):
    ...
    def _get_view(self,data,sheet,context=None):
        s = "select coalesce(u.alias,u.name,'') as loc_name, \
            ...
            sum(-g.product_uom_qty) as product_uom_qty, \
            b.product_uom, \
            ...
            a.date_order, \
            min(coalesce(ae.name,'')) as tracking_name, \
            avg((case when coalesce(x.quantity,0.0)>0.0 then coalesce(x.price_subtotal,0.0)/coalesce(x.quantity,0.0) \
                else 0.0 end)) as cury_net_price, \
            avg((case when coalesce(x.quantity,0.0)>0.0 then (coalesce(x.price_subtotal,0.0)/coalesce(x.quantity,0.0)) * \
                ...
                else 0.0 end)) as net_price, \
            avg(coalesce(y.tax_percent,0.0)) as tax_percent, \
            avg(coalesce(x.price_unit,0.0)) as cury_sell_price, \
            avg(coalesce(x.price_unit,0.0) * \
                case when coalesce(z2.name,'') = 'IDR' then \
                    case when coalesce(z.name,'') = 'IDR' then 1.0 / coalesce(ao.rate,1.0) \
                    else coalesce(ad.rate,0.0) / coalesce(ao.rate,1.0) end \
                else 1.0 / coalesce(ab.rate,1.0) end) as sell_price, \
            ...
            avg(coalesce(x.price_unit,0.0) * \
                ...
                else coalesce(am.rate,0.0) / coalesce(ab.rate,1.0) end) as sell_price_idr, \
            avg(coalesce(x.price_unit,0.0) * \
                case when coalesce(z2.name,'') = 'IDR' then \
                    case when coalesce(z.name,'') = 'IDR' then 1.0 / coalesce(ak.rate,1.0) \
                    else coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end \
                else coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end) as sell_price_usd, \
            sum(coalesce(x.price_subtotal,0.0)) as cury_net_amount, \
            sum(coalesce(x.price_subtotal,0.0) * \
                case when coalesce(z2.name,'') = 'IDR' then \
                    case when coalesce(z.name,'') = 'IDR' then 1.0 / coalesce(ao.rate,1.0) \
                    else coalesce(ad.rate,0.0) / coalesce(ao.rate,1.0) end \
                else 1.0 / coalesce(ab.rate,1.0) end) as net_amount, \
            sum(coalesce(x.tax_amount,0.0)) as cury_tax_amount, \
            sum(coalesce(x.tax_amount,0.0) * \
                case when coalesce(z2.name,'') = 'IDR' then \
                    case when coalesce(z.name,'') = 'IDR' then 1.0 / coalesce(ao.rate,1.0) \
                    else coalesce(ad.rate,0.0) / coalesce(ao.rate,1.0) end \
                else 1.0 / coalesce(ab.rate,1.0) end) as tax_amount, \
            sum(coalesce(x.price_subtotal,0.0)+coalesce(x.tax_amount,0.0)) as cury_tot_amount, \
            sum((coalesce(x.price_subtotal,0.0)+coalesce(x.tax_amount,0.0)) * \
                case when coalesce(z2.name,'') = 'IDR' then \
                    case when coalesce(z.name,'') = 'IDR' then 1.0 / coalesce(ao.rate,1.0) \
                    else coalesce(ad.rate,0.0) / coalesce(ao.rate,1.0) end \
                else 1.0 / coalesce(ab.rate,1.0) end) as tot_amount, \
            sum((coalesce(x.price_subtotal,0.0)+coalesce(x.tax_amount,0.0)) * \
                case when coalesce(z2.name,'') = 'IDR' then \
                    case when coalesce(z.name,'') = 'IDR' then 1.0 \
                    else coalesce(ak.rate,0.0) * coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end \
                else coalesce(am.rate,0.0) / coalesce(ab.rate,1.0) end) as tot_amount_idr, \
            sum((coalesce(x.price_subtotal,0.0)+coalesce(x.tax_amount,0.0)) * \
                case when coalesce(z2.name,'') = 'IDR' then \
                    case when coalesce(z.name,'') = 'IDR' then 1.0 / coalesce(ak.rate,1.0) \
                    else coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end \
                else coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end) as tot_amount_usd, \
            sum(case when a.sale_type = 'local' and a.locale_sale_type = 'ikb' then 0.0 \
                else case when coalesce(z2.name,'') = 'IDR' then \
                        case when coalesce(z.name,'') = 'IDR' then coalesce(x.price_subtotal,0.0) \
                        else coalesce(x.price_subtotal,0.0) * coalesce(ak.rate,0.0) * coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end \
                     else coalesce(x.price_subtotal,0.0) * coalesce(am.rate,0.0) / coalesce(ab.rate,1.0) end \
                end) as dpp_npet, \
            sum(case when a.sale_type = 'local' and a.locale_sale_type = 'ikb' then \
                    case when coalesce(z2.name,'') = 'IDR' then \
                        case when coalesce(z.name,'') = 'IDR' then coalesce(x.price_subtotal,0.0) \
                        else coalesce(x.price_subtotal,0.0) * coalesce(ak.rate,0.0) * coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end \
                    else coalesce(x.price_subtotal,0.0) * coalesce(am.rate,0.0) / coalesce(ab.rate,1.0) end \
                else 0.0 end) as dpp_pet, \
            sum(case when a.sale_type = 'local' and a.locale_sale_type = 'ikb' then 0.0 \
                else case when coalesce(z2.name,'') = 'IDR' then \
                        case when coalesce(z.name,'') = 'IDR' then coalesce(x.price_subtotal,0.0)*0.1 \
                        else coalesce(x.price_subtotal,0.0) * 0.1 * coalesce(ak.rate,0.0) * coalesce(ai.rate,0.0) / coalesce(ab.rate,1.0) end \
                     else coalesce(x.price_subtotal,0.0) * 0.1 * coalesce(am.rate,0.0) / coalesce(ab.rate,1.0) end \
                end) as tax_npet, \
            0.0 as tax_pet \
            from (  select g1.sale_id,g1.name,g1.date_done,g1.invoice_id,g2.sequence_line,g2.sale_line_id, \
                    ...
                    group by g1.sale_id,g1.name,g1.date_done,g1.invoice_id,g2.sequence_line,g2.sale_line_id, \
            ...
            left join account_invoice_line x on g.invoice_id = x.invoice_id and g.invoice_line_id = x.id \
        ...
        if sheet == 'customer':
            if data['form']['sale_type'] == 'export':
                self.cr.execute (s + " where a.goods_type = %s \
                                       and a.sale_type = %s \
                                       group by coalesce(u.alias,u.name,''),coalesce(d.partner_alias,d.name),g.name,to_char(g.date_done,'YYYY-MM-DD'), \
                                       to_char(g.date_done,'DD/MM/YYYY'),v.internal_number,coalesce(w.nomor_urut,''),coalesce(q.name,''), \
