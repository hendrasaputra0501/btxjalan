...
class outstanding_commision_parser(report_sxw.rml_parse):
	def __init__(self, cr, uid, name, context=None):
		super(outstanding_commision_parser, self).__init__(cr, uid, name, context=context)
	...
	def _get_result(self, data):
		res = []
		as_on =data['form']['as_on'].encode("utf-8")
		sale_type = data['form']['sale_type'].encode("utf-8")
		fiscalyear = data['form']['fiscalyear_id'] and self.pool.get('account.fiscalyear').browse(self.cr, self.uid, data['form']['fiscalyear_id'][0]) or False
		start_date = fiscalyear and fiscalyear.date_start or '2015-01-01'

		query = """SELECT * FROM commission_detail_outstanding('"""+start_date+"""'::date,'"""+as_on+"""'::date,'"""+sale_type+"""'::text)"""
		if fiscalyear:
			query = query + " WHERE to_char(inv_date,'YYYY-MM-DD') between '"+start_date+"' AND '"+fiscalyear.date_stop+"'"
...
class outstanding_commision_xls(report_xls):
	...
	def generate_xls_report(self, parser, data, obj, wb):
		...
		ws = wb.add_sheet("Outstanding Commission",cell_overwrite_ok=True)
		...
		ws.write_merge(1,1,0,15, "OUTSTANDING COMMISSION COST", title_style)
		ws.write_merge(2,2,0,15, "AS ON "+datetime.strptime(data['form']['as_on'],"%Y-%m-%d").strftime("%d/%m/%Y"), title_style)
		...
		total_insurance = 0.0
		total_fob = 0.0
		total_paid_amt_usd = 0.0
		# group by agent
		result_grouped2 = {}
		for line in sorted(result, key=lambda l:(l['inv_date'],l['inv_no'])):
			if not (line['comm_amt_fob']-line['paid_amt_usd']) or (line['comm_amt_fob']-line['paid_amt_usd'])<=0.0:
				continue
			ws.write(rowcount,0,line['inv_no'],normal_style)
			ws.write(rowcount,1,line['inv_date'] and parser.formatLang(line['inv_date'],date=True) or '',normal_style)
			...
			ws.write(rowcount,14,line['comm_amt_fob']-line['paid_amt_usd'],normal_right_style1)
			ws.write(rowcount,15,line['incoterm'],normal_style)
			total_inv += line['price_subtotal']
			total_comm_amt += line['comm_amt']
			total_freight += line['freight']
			total_insurance += line['insurance']
			total_fob += line['fob']
			total_paid_amt_usd += line['comm_amt_fob']-line['paid_amt_usd']
			rowcount+=1
			...
			result_grouped2.update({
				key3:{
					'comm_amount': result_grouped2[key3]['comm_amount']+line['comm_amt'],
					'comm_outs': result_grouped2[key3]['comm_outs']+line['comm_amt_fob']-line['paid_amt_usd'],
				}
			})
		...
		ws.write(rowcount,14,total_paid_amt_usd,subtotal_style1)
		if total_paid_amt_usd and len(str(total_paid_amt_usd))>max_width_col[14]:
			max_width_col[14]=len(str(total_paid_amt_usd))
		...
		rowcount+=2
		ws.write_merge(rowcount,rowcount,9,10,"",hdr_style_border_bottom)
		ws.write(rowcount,11,"Commision",hdr_style_border_bottom)
		rowcount+=1
		total_comm_amt2, total_paid_amt_usd2 = 0.0, 0.0
		for key4 in sorted(result_grouped2.keys()):
			ws.write_merge(rowcount,rowcount,9,10,key4,normal_style)
			ws.write(rowcount,11,result_grouped2[key4]['comm_amount'],normal_right_style1)
			ws.write(rowcount,12,result_grouped2[key4]['comm_outs'],normal_right_style1)
			total_comm_amt2+=result_grouped2[key4]['comm_amount']
			total_paid_amt_usd2+=result_grouped2[key4]['comm_outs']
			rowcount+=1
		ws.write_merge(rowcount,rowcount,9,10, "Total", subtotal_style)
		ws.write(rowcount,11,total_comm_amt2,subtotal_style1)
		ws.write(rowcount,12,total_paid_amt_usd2,subtotal_style1)
		rowcount+=1
		pass

outstanding_commision_xls('report.outstanding.commision.report', 'outstanding.commision.wizard', 'addons/ad_sales_report/report/pending_sales_report.mako', parser=outstanding_commision_parser, header=False)