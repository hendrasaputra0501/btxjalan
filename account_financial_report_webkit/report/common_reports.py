...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def sort_accounts_with_structure(self, root_account_ids, account_ids,
        ...
        def recursive_sort_by_code(accounts, parent):
            ...
            level_accounts = sorted(level_accounts, key = lambda x:x['code2'] or x['code'] or x['id'])
        ...
        # change this when you install this menu to other company
        accounts_data = self.pool.get('account.account').read(
            self.cr, self.uid, account_ids,
            ['id', 'parent_id', 'level', 'code', 'child_consol_ids', 'code2'],
            context=context)
        ...
        for root_account_data in sorted(root_accounts_data, key = lambda x:x['code2'] or x['code'] or x['id']):
    ...
    def _get_opening_periods(self, fiscalyear_id=False):
        ...
        if fiscalyear_id:
            return self.pool.get('account.period').search(self.cursor, self.uid,[('special', '=', True),('fiscalyear_id', '=', fiscalyear_id)])
    ...
    def _compute_init_balance_from_date(self, account_id=None, start_date=None,
                              mode='computed', opening_period=None, default_values=False):
        res = {}

        if not default_values:
            if not account_id or not start_date or not opening_period:
                raise Exception('Missing account or start_date or opening period')
            try:
                if mode == 'read':
                    query = "SELECT sum(debit) AS debit, \
                            ...
                            WHERE period_id = %s \
                                AND account_id = %s"
                    query = query%(opening_period.id, account_id)
                else:
                    query = "SELECT sum(debit) AS debit, \
                                sum(credit) AS credit, \
                                sum(debit)-sum(credit) AS balance, \
                            ...
                            WHERE date >= '%s' \
                                AND date < '%s' \
                                AND account_id = %s"
                    query = query%(opening_period.date_start, start_date, account_id)
    ...
    def _compute_initial_balances_from_date(self, account_ids, start_date, fiscalyear):
        opening_period_id = self._get_opening_periods(fiscalyear_id=fiscalyear.id)
        if not opening_period_id:
            raise osv.except_osv(
                ...
                _('No opening period found to compute the opening balances.\n'
                  ...
                  ' with the special flag.'))
        res = {}
        opening_period = self.pool.get('account.period').browse(self.cursor,self.uid,opening_period_id[0])
        for acc in self.pool.get('account.account').browse(self.cursor,self.uid,account_ids):
            res[acc.id] = self._compute_init_balance_from_date(default_values=True)
            if opening_period.date_start == start_date:
                res[acc.id] = self._compute_init_balance_from_date(acc.id, start_date, mode='read', opening_period=opening_period)
            elif opening_period.date_start < start_date:
                res[acc.id] = self._compute_init_balance_from_date(acc.id, start_date, opening_period=opening_period)
        return res
