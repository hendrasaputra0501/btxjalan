...
class account_bill_passing(osv.Model):
	...
	def action_approve(self, cr, uid, ids, context=None):
		...
		for bill in self.browse(cr, uid, ids, context=context):
			# fill the partner_id if its empty and have a link with invoice.commission
			for x in bill.bill_lines:
				if not x.partner_id and (x.comm_id or x.comm_provision_id):
					if x.comm_id:
						x.write({'partner_id':x.comm_id.agent_id.id})
						comm_ids.append(x.comm_id.id)
					else:
						x.write({'partner_id':x.comm_provision_id.agent_id.id})
						comm_ids.append(x.comm_provision_id.id)
			...
			for line in bill_lines:
				...
				if curr_partner_inv.get(line.get('partner_id',False),False):
					key2 = curr_partner_inv[line['partner_id']]
			...
			# to update paid amount of provisioned outs comm
			to_update_inv_ids = []
			for key_inv_id in bill_lines_grouped1.keys():
				...
				for line in bill_lines:
					...
					if not (line.comm_id or False) and (line.comm_provision_id or False):
						aic_obj.write(cr, uid, line.comm_provision_id.id, {
								'invoice_prov_id':key_inv_id,
								'invoice_prov_line_id':inv_line_id, 
								'bill_prov_id':line.id})
			...
			for key in bill_lines_grouped2.keys():
				...
				for line in bill_lines:
					...
					if not (line.comm_id or False) and (line.comm_provision_id or False):
						aic_obj.write(cr, uid, line.comm_provision_id.id, {
							'invoice_prov_id':inv_id,
							'invoice_prov_line_id':inv_line_id, 'bill_prov_id':line.id})
		...
		aic_obj.write(cr, uid, comm_ids, {'bill_ids':[]})
		return True
...
class account_bill_passing_line(osv.Model):
	...
	def _get_amount_tax(self, cr, uid, ids, prop, unknow_none, unknow_dict):
		...
		for line in self.browse(cr, uid, ids):
			price = line.amount
			taxes = tax_obj.compute_all(cr, uid, line.bill_line_tax_id, price, line.qty, product=False, partner=line.partner_id)
	...
	_columns = {
		...
		'remark' : fields.char('Remark',size=320),
	}
