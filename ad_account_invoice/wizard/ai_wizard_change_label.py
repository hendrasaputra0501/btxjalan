...
class ai_wizard_change_label(osv.osv_memory):
	_name = "ai.wizard.change.label"
	_columns = {
		'field_ids':fields.one2many('ai.wizard.change.label.line','wizard_id','Fields'),
	}
	...
	def default_get(self, cr, uid, fields_list, context=None):
		...
		"""
		res = super(ai_wizard_change_label, self).default_get(cr, uid, fields_list, context=context)
		if context is None:
			context = {}
	...
	def set_label_print(self, cr, uid, ids, context=None):
		...
		if not data:
			return False
		data_line = self.pool.get('ai.wizard.change.label.line').browse(cr, uid, data[0].get('field_ids',[]))
		if not data_line:
			return False
		result={}
		for line in data_line:
			result.update({line.field_id.key.encode("utf-8"):line.label.encode("utf-8")})
		self.pool.get('account.invoice').write(cr,uid,context['active_id'],{'label_print':str(result)})
		return True

ai_wizard_change_label()

class ai_wizard_change_label_line(osv.osv_memory):
	_name = "ai.wizard.change.label.line"
	_rec_name ="field_id"
	_columns = {
		'field_id':fields.many2one('label.print','name'),
		'label':fields.char('Label',size=100),
		'wizard_id':fields.many2one('ai.wizard.change.label','Wizard')
	}
ai_wizard_change_label()