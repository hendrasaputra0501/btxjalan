...
class account_statement_from_invoice_lines(osv.osv_memory):
	...
	def populate_statement(self, cr, uid, ids, context=None):
		...
		line_date = statement.date!=False and statement.date or time.strftime('%Y-%m-%d')
		...
		for line in line_obj.browse(cr, uid, line_ids, context=context):
			...
			statement_line_obj.create(cr, uid, {
				...
				'partner_bank_id':line.partner_id.bank_ids and line.partner_id.bank_ids[0].id or False,
			}, context=context)
		statement_obj.write(cr, uid, [statement.id], {'line_ids':[]})
		return {'type': 'ir.actions.act_window_close'}
