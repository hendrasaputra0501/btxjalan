...
class wizard_kbkb_charge_onshipment(osv.osv_memory):
	...
	_name = "wizard.kbkb.charge.onshipment"
	_description = "Wizard KBKB Charge OnShipment"

	_columns = {
		'journal_id': fields.selection(_get_journal_id, 'Destination Journal',required=True),
		'currency_id' : fields.many2one('res.currency','Currency',required=True),
		'bpa_date': fields.date('BPA Request Date',required=True),
		'due_date': fields.date('BPA Due Date',required=False),
	...
	def view_init(self, cr, uid, fields_list, context=None):
		if context is None:
			context = {}
		res = super(wizard_kbkb_charge_onshipment, self).view_init(cr, uid, fields_list, context=context)
		...
		for pick in pick_obj.browse(cr, uid, active_ids, context=context):
			...
			if not pick.porters:
				raise osv.except_osv(_('Warning!'), _('Please define the Porters first'))

			if not pick.porters_charge:
				raise osv.except_osv(_('Warning!'), _('Please define the Porters Charge first'))
	...
	def open_kbkb_bpa(self, cr, uid, ids, context=None):
		if context is None:
			context = {}
		bpa_ids = []
		data_pool = self.pool.get('ir.model.data')
		res = self.create_kbkb_bpa(cr, uid, ids, context=context)
		bpa_ids += res.values()
		action_model = False
		action = {}
		if not bpa_ids:
			raise osv.except_osv(_('Error!'), _('Please create KBKB BPA.'))
	...
	def create_kbkb_bpa(self, cr, uid, ids, context=None):
		if context is None:
			context = {}
		picking_pool = self.pool.get('stock.picking')
		onshipdata_obj = self.read(cr, uid, ids, ['journal_id', 'group', 'bpa_date', 'currency_id','due_date','number'])
		...
		res = picking_pool.action_kbkb_bpa_create(cr, uid, active_ids,
			  journal_id = onshipdata_obj[0]['journal_id'],
			  context=context)
		return res

wizard_kbkb_charge_onshipment()
