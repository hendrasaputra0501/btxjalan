...
class stock_picking(osv.osv):
    ...
    def do_partial(self, cr, uid, ids, partial_datas, context=None):
        ...
        """
        # print "=================xxxxxxxxxxxxxxgggggggggg"
        if context is None:
            context = {}
        else:
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        moves = []
        if ('move_lines' not in default.keys()) and picking.move_lines:
            for move in picking.move_lines:
                move_dic = self.pool.get('stock.move').copy_data(cr, uid, move.id)
                if move_dic.get('matching_ids_in',{}):
                    del move_dic['matching_ids_in']
                if move_dic.get('matching_ids_out',{}):
                    del move_dic['matching_ids_out']
                moves.append((0,0,move_dic))
            default.update({'move_lines':moves})
...
class stock_move(osv.osv):
    ...
    _columns = {
        ...
        'price_unit': fields.float('Unit Price', digits=(16,8), help="Technical field used to record the product cost set by the user during a picking confirmation (when average price costing method is used)"),
    }
                    ...
                    for j_id, move_lines in account_moves_neg:
                        move_obj.create(cr, uid,
                                        {'journal_id': j_id, 
                                         ...
                                         'date': out_mov.picking_id and datetime.strptime(out_mov.picking_id.date_done,'%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d') or time.strftime('%Y-%m-%d'),
                                         'period_id' : self.pool.get('account.period').find(cr, uid, (out_mov.picking_id and datetime.strptime(out_mov.picking_id.date_done,'%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d') or time.strftime('%Y-%m-%d')), context={'special':False}) and self.pool.get('account.period').find(cr, uid, (out_mov.picking_id and datetime.strptime(out_mov.picking_id.date_done,'%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d') or time.strftime('%Y-%m-%d')), context={'special':False})[0] or False,
                                         })

            move_date = move.picking_id and move.picking_id.date_done!=False and \
                    datetime.strptime(move.picking_id.date_done,'%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d') or (\
                    move.date!=False and datetime.strptime(move.date,'%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d') or time.strftime('%Y-%m-%d'))
                ...
                move_obj.create(cr, uid,
                        {
                         ...
                         'date': move_date,
                         'period_id' : self.pool.get('account.period').find(cr, uid, move_date, context={'special':False}) and self.pool.get('account.period').find(cr, uid, move_date, context={'special':False})[0] or False,
                            })
    ...
    def _create_account_move_line(self, cr, uid, move, matches, src_account_id, dest_account_id, reference_amount, reference_currency_id, type='', context=None):
        ...
        #print "########################_create_account_move_line",reference_amount
        move_list = []
    ...
    def price_calculation(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if move.location_id.usage == 'internal' and move.location_dest_id.usage != 'internal':
                ...
                ctx.update({'location_dest_id':move.location_id.id})
                ...
                price_amount = 0.0
                amount = 0.0
                # print "=========Standard Price New DxO=========",tuples
                ...
                if product_avail[product.id] >= product_uom_qty and product.cost_method in ['fifo', 'lifo']:
                    if amount > 0:
                        ...
                        # print "=========Standard Price New DO=========",price_amount / amount
                    else:
            ...
            if move.location_id.usage != 'internal' and move.location_dest_id.usage == 'internal':
                if move.price_unit == 0.0:
                    new_price = uom_obj._compute_price(cr, uid, product.uom_id.id, product.standard_price, move_uom)
                    self.write(cr, uid, move.id, {'price_unit': new_price}, context=ctx)
                    # print "=========Standard Price New=========",new_price
                    ...
                    # print "------------Standard Price------------->",price_amount / product_uom_qty
