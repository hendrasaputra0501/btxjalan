...
class product_product (osv.osv):
    ...
    def get_stock_matchings_fifolifo(self, cr, uid, ids, qty, fifo, product_uom_id=False, currency_id=False, context=None):
        ...
        lot_ids = context.get('lot_ids',False)
        if not lot_ids:
            ...
            if context.get('location_dest_id',False) :
                # domain.append(('location_id', '!=', context.get('location_dest_id',False)))
                domain.append(('location_dest_id', '=', context.get('location_dest_id',False)))
            if context.get('to_date',False):
                domain.append(('date', '<=', context.get('to_date',False)))

            if product.internal_type == 'Raw Material':
                domain.append(('tracking_id', '=', context.get('tracking_id',False)))
            move_in_ids = move_obj.search(cr, uid, domain, order = order, context=context)
            # use this code if you want to use the original method
        ...
        for move in move_obj.browse(cr, uid, move_in_ids, context=context):
            ...
            if qty_to_go - product_qty >= 0: 
                tuples.append((move.id, product_qty, new_price, qty_from, date_move),)
                qty_to_go -= product_qty
            else:
                # tuples.append((move.id, qty_to_go, new_price, qty_from * qty_to_go / product_qty),)
                tuples.append((move.id, qty_to_go, new_price, qty_from, date_move),)
                qty_to_go = 0
                break
        if product.cost_method in ('fifo','lifo') and round(qty_to_go,4) > 0:
            if round(qty_to_go,4) == round(qty,4):
                raise osv.except_osv(_('Stock Matching (FIFO/LIFO) Error'), _("Something went wrong finding stock moves for Product Code "+product.default_code+" ("+str(context.get('move_out_id',''))+"). This product is not available in its source location"))
            else:
                raise osv.except_osv(_('Stock Matching (FIFO/LIFO) Error'), _("Something went wrong finding stock moves for Product Code "+product.default_code+" ("+str(context.get('move_out_id',''))+"). This product is not be able to be fully issued yet. Remaining Qty to Issue :"+str(qty_to_go)))
...
class stock_move(osv.osv):
    ...
    def _get_qty_remaining (self, cr, uid, ids, field_names, arg, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if (move.location_id.usage != 'internal' or move.location_id.usage=='internal') and move.location_dest_id.usage == 'internal':
    ...
    _columns = {'qty_remaining': fields.function(_get_qty_remaining, digits_compute=dp.get_precision('Product Unit of Measure'), type="float", string="Remaining quantity to be matched", 
                                                 store = {'stock.move.matching': (_get_moves_from_matchings, ['qty', 'move_in_id', 'move_out_id'], 10),
                                                          'stock.move':  (lambda self, cr, uid, ids, ctx: ids, ['product_qty', 'product_uom', 'location_id', 'location_dest_id', 'matching_ids_in', 'matching_ids_out'], 10)}),
...
class stock_move_matching(osv.osv):
    ...
    def _get_unit_price_out (self, cr, uid, ids, field_names, arg, context=None):
        ...
        for match in self.browse(cr, uid, ids, context=context):
            if match.move_in_id and match.move_out_id:
    ...
    _columns = {
        'move_in_id': fields.many2one('stock.move', 'Stock move in', required=True,select=True),
        'move_out_id': fields.many2one('stock.move', 'Stock move out', required=True,select=True),
        'qty': fields.float('Quantity in UoM of out', digits_compute=dp.get_precision('Product Unit of Measure') ,required=True), 
        'price_unit':fields.related('move_in_id', 'price_unit', digits=(16,4), string="Unit price", type="float"),
        'price_unit_out': fields.function(_get_unit_price_out, digits=(16,4), type="float", string="Price in UoM of out move", 
                                                 store = {'stock.move': (_get_matches, ['price_unit', 'product_uom','matching_ids_in','matching_ids_out'], 10), 
