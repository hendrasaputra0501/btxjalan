...
class stock_partial_picking(osv.osv_memory):
    ...
    def _product_cost_for_average_update(self, cr, uid, move):
        ...
        if move.picking_id.purchase_id:
            ...
            date = move.picking_id.date_done and datetime.strptime(move.picking_id.date_done,'%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d') or (datetime.strptime(move.date,'%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d'))
            ctx = {'date':date}
            try:
                price_unit = self.pool.get('purchase.order.line').get_line_price_after_disc(cr,uid,move.purchase_line_id)
                taxes = tax_obj.compute_all(cr, uid, move.purchase_line_id.taxes_id, price_unit, move.product_qty, move.product_id, move.purchase_line_id.order_id.partner_id)
                order_id = move.purchase_line_id.order_id or move.purchase_line_id.old_order_id
                cur = order_id.pricelist_id.currency_id
                amount = currency_obj.round(cr, uid, cur, taxes['total'])
                amount = currency_obj.compute(cr, uid, move.picking_id.purchase_id.currency_id.id, move.company_id.currency_id.id, 
                                                 amount, round=False, context=ctx)
                new_price = amount/move.product_qty
            except:
                new_price = currency_obj.compute(cr, uid, move.picking_id.purchase_id.currency_id.id, move.company_id.currency_id.id, 
                                                 move.price_unit, round=False, context=ctx)
